...
contract Taker is MerkleOrderTaker {
    ...
    // widthdraw gas
    function withdrawGasFromSettler(address settler, address dest) external {
        MerkleOrderSettler merkle = MerkleOrderSettler(payable(settler));

        merkle.withdrawGas(payable(dest), merkle.prepaidGas(address(this)));
    }
}